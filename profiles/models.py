...
def create_or_update_customer_profile(sender, instance, created, **kwargs):
    ...
    When a new User instance is created, a corresponding CustomerProfile instance
    is created. If an existing User instance is saved (updated), the associated
    CustomerProfile instance is updated.

    Args:
        sender (type): The model class that sent the signal (User in this case).
        instance (User): The actual instance of the User model that was saved.
